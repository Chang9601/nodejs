/* 
  ...
  이미터와 리스너를 사용하는 패턴을 관찰자(observer) 패턴이라고 한다.
*/
const EventEmitter = require('events');

// HTTP, 파일 시스템 및 많은 NodeJS 핵심 모듈은 내부적으로 이벤트를 구현하기 위해 EventEmitter 클래스를 상속한다.
class Meal extends EventEmitter {
  constructor() {
    super();
  }
}

const eventEmitter = new Meal(); // new EventEmitter();
...
/*
  ...
  함수를 호출할 때마다 해당 함수의 실행 컨텍스트가 호출 스택의 맨 위에 푸시되고 함수가 종료되면 실행 컨텍스트가 호출 스택에서 제거된다.
...
eventEmitter.on('meal', function () {
  console.log('저녁은 건강을 위해 굶는다!');
});

/*
  emit() 메서드는 명시된 이름의 이벤트를 발생시키고 모든 EventEmmiter 클래스 인스턴스가 해당 이벤트의 리스너를 호출한다.
  emit() 메서드를 사용하면 리스너 함수에 인자를 전달할 수 있으며 리스너 함수 내에서 this 예약어 EventEmitter 클래스의 인스턴스를 참조한다.
*/
